...
ID3D11Buffer *gpVertexBufferCubeCol = NULL;

ID3D11Buffer *gpVertexBufferPyramidPos = NULL;
ID3D11Buffer *gpVertexBufferPyramidCol = NULL;
...
HRESULT initialize(void) {
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeColor);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferCubeCol);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Cube Color!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Cube Color Successful!!.\n");
        fclose(fptr);
    }
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferCubeCol, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, cubeColor, sizeof(cubeColor));
    gpID3D11DeviceContext->Unmap(gpVertexBufferCubeCol, 0);
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidColor);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferPyramidCol);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Pyramid Color!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Pyramid Color Successful!!.\n");
        fclose(fptr);
    }
    //Copy Verteces into Abover Buffer Using MAPPING
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferPyramidCol, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, pyramidColor, sizeof(pyramidColor));
    gpID3D11DeviceContext->Unmap(gpVertexBufferPyramidCol, 0);
...
void display(void){
    ...
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferCubeCol, &iStride, &iOffset);
    ...
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferPyramidCol, &iStride, &iOffset);
...
void uninitialize(void) {
    ...
    if(gpVertexBufferCubeCol){
        gpVertexBufferCubeCol->Release();
        gpVertexBufferCubeCol = NULL;
    }
    ...
    if(gpVertexBufferPyramidCol){
        gpVertexBufferPyramidCol->Release();
        gpVertexBufferPyramidCol = NULL;
    }
