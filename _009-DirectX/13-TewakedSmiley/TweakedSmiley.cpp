...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "__TweakedSmileyLog.txt";
...
HRESULT initialize(void) {
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * 2 * 6;
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
 ...
 */
    ...
    gpID3D11DeviceContext->RSSetState(gpRasterizerState);
        ...
        //Pyramid!.
    hr = loadD3DTexture(L"Smiley.bmp", &gpSRVRectTex);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
...
void setTexBufferData(int iKey) {
    float rectTexCoords[12];
    ...
    if(iKey == 1) {
        ...
        rectTexCoords[0] = 0.0f;
        rectTexCoords[1] = 4.0f;
        rectTexCoords[2] = 4.0f;
        rectTexCoords[3] = 4.0f;
        rectTexCoords[4] = 0.0f;
        rectTexCoords[5] = 0.0f;
        rectTexCoords[6] = 0.0f;
        rectTexCoords[7] = 0.0f;
        rectTexCoords[8] = 4.0f;
        rectTexCoords[9] = 4.0f;
        rectTexCoords[10] = 4.0f;
        rectTexCoords[11] = 0.0f;
    }
    else{
