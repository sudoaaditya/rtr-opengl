...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_PerVertexLightOnSphereLog.txt";
...
HRESULT initialize(void) {
    ...
    const char* vertexShaderSourceCode =
        ...
        "       float3 tNorm = normalize(mul(mul((float3x3)worldMat, (float3x3)viewMat), (float3)normal));"   \
        "       float3 lightDir = (float3)normalize(lightPos - eyeCoords);"    \
        "       float tn_dot_ld = max(dot(lightDir, tNorm), 0.0);"  \
        "       float4 ambient = la * ka;"  \
        "       float4 diffuse = ld * kd * tn_dot_ld;"  \
        "       float3 reflectionVec = reflect(-lightDir, tNorm);"  \
        "       float3 viewerVec = normalize(-eyeCoords.xyz);"  \
        "       float4 specular = ls * ks * pow(max(dot(reflectionVec, viewerVec), 0.0), matShine);"    \
        "       vertOut.phong_ads_light = ambient + diffuse + specular;"  \
        "   }"
        "   else {" 
        "       vertOut.phong_ads_light = float4(1.0, 1.0, 1.0, 1.0);" \
        "   }"
    ...
    const char* pixelShaderSourceCode = 
        "float4 main(float4 position: SV_POSITION, float4 phong_ads_light_color: COLOR) : SV_TARGET {" \
