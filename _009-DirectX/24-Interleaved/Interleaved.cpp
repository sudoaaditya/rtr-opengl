...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_InterleavedLog.txt";
...
ID3D11Buffer *gpVertexBufferCube = NULL;
...
HRESULT initialize(void) {
    ...
    const char* vertexShaderSourceCode =
        ...
        "};" \
        "Vertex_Output main( float4 pos: POSITION, float4 color: COLOR, float4 normal : NORMAL, float2 texCoords : TEXCOORD ) {"   \
    ...
    const char* pixelShaderSourceCode = 
        ...
        "   }"  \
        "   float4 tex = myTex2D.Sample(texSamplerState, pixelIn.texCoords);"
        "   return(tex * pixelIn.color * phong_ads_color);"   \
        "}";
    ...
    inputElementsDesc[2].InputSlot = 2; // AMC_ATTRIBUTE_NORMAL = 2
    inputElementsDesc[2].AlignedByteOffset = 0;
    ...
    inputElementsDesc[3].Format = DXGI_FORMAT_R32G32_FLOAT; // 2Float
    inputElementsDesc[3].InputSlot = 3; // AMC_ATTRIBUTE_TEXCOORD = 3
    inputElementsDesc[3].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;  //GL_UNPACK_ALIGNMENT : 4
    inputElementsDesc[3].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
    inputElementsDesc[3].InstanceDataStepRate = 0;

    hr = gpID3D11Device->CreateInputLayout(inputElementsDesc, 4, 
    ...
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeVCNTCoords);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferCube);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Cube!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Cube Successful!!.\n");
        fclose(fptr);
    }
    //Copy Verteces into Above Buffer Using MAPPING
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferCube, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, cubeVCNTCoords, _ARRAYSIZE(cubeVCNTCoords));
    gpID3D11DeviceContext->Unmap(gpVertexBufferCube, 0);
    ...
    //Texture Creation
    hr = loadD3DTexture(L"Marble.bmp", &gpSRVCubeTex);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
...
HRESULT loadD3DTexture(const wchar_t *texFileName, ID3D11ShaderResourceView **ppTexShaderRes) {
    ...
    //Create Shader.
    hr = DirectX::CreateWICTextureFromFile(gpID3D11Device, gpID3D11DeviceContext, texFileName, NULL, ppTexShaderRes);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
...
void display(void){
    ...
    //Set Position
    UINT iStride = sizeof(float) * 11;
    UINT iOffset = sizeof(float) * 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferCube, &iStride, &iOffset);
    //Set Color
    iStride = sizeof(float) * 11;
    iOffset = sizeof(float) * 3;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferCube, &iStride, &iOffset);
    //Set Normal
    iStride = sizeof(float) * 11;
    iOffset = sizeof(float) * 6;
    gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpVertexBufferCube, &iStride, &iOffset);
    //Set Texture
    iStride = sizeof(float) * 11;
    iOffset = sizeof(float) * 9;
    gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpVertexBufferCube, &iStride, &iOffset);
...
void uninitialize(void) {
    ...
    if(gpVertexBufferCube){
        gpVertexBufferCube->Release();
        gpVertexBufferCube = NULL;
    }
