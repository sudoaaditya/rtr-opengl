...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_PersTriangleLog.txt";
...
ID3D11Buffer *gpVertexBufferPos = NULL;
ID3D11Buffer *gpVertexBufferCol = NULL;
...
HRESULT initialize(void) {
    ...
    inputElementsDesc[1].InputSlot = 1; //Color at slot 1 in buffer like AMC_ATTRIBUTE_COLOR = 1.
    ...
    D3D11_BUFFER_DESC vertexBufferPosition;
    ZeroMemory(&vertexBufferPosition, sizeof(D3D11_BUFFER_DESC));
    vertexBufferPosition.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferPosition.ByteWidth = sizeof(float) * _ARRAYSIZE(triangleVert);
    vertexBufferPosition.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferPosition.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferPosition, NULL, &gpVertexBufferPos);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer Position!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer Position Successful!!.\n");
        fclose(fptr);
    }
    ...
    gpID3D11DeviceContext->Map(gpVertexBufferPos, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, triangleVert, sizeof(triangleVert));
    gpID3D11DeviceContext->Unmap(gpVertexBufferPos, 0);
    ...
    D3D11_BUFFER_DESC vertexBufferColor;
    ZeroMemory(&vertexBufferColor, sizeof(D3D11_BUFFER_DESC));
    vertexBufferColor.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferColor.ByteWidth = sizeof(float) * _ARRAYSIZE(triangleCol);
    vertexBufferColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&vertexBufferColor, NULL, &gpVertexBufferCol);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer Color!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer Color Successful!!.\n");
        fclose(fptr);
    }

    D3D11_MAPPED_SUBRESOURCE mappedResCol;
    ZeroMemory(&mappedResCol, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferCol, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResCol);
    memcpy(mappedResCol.pData, triangleCol, sizeof(triangleCol));
    gpID3D11DeviceContext->Unmap( gpVertexBufferCol, 0);
...
void display(void){
    ...
    UINT iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferPos, &iStride, &iOffset);
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferCol, &iStride, &iOffset);
...
void uninitialize(void) {
    ...
    //Code!.
        ...
        if(gpVertexBufferPos){
        gpVertexBufferPos->Release();
        gpVertexBufferPos = NULL;
    }

    if(gpVertexBufferCol){
        gpVertexBufferCol->Release();
        gpVertexBufferCol = NULL;
    }
