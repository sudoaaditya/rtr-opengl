...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "__CheckerBoardLog.txt";
...
ID3D11Buffer *gpVertexBufferRectanglePos = NULL;
ID3D11Buffer *gpVertexBufferRectangleTex = NULL;
//ShaderResourceView & SamplerState
ID3D11ShaderResourceView *gpSRVRectTex = NULL;
ID3D11SamplerState *gpSamplerStateRectTex = NULL;
...
uint8_t CheckerBoardData[CHECKER_WIDTH *CHECKER_HEIGHT * 4];
...
HRESULT initialize(void) {
    ...
    vertexBufferDesc.ByteWidth = sizeof(float) * 6 * 3;
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferRectanglePos);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Rectangle Position!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Rectangle Position Successful!!.\n");
        fclose(fptr);
    }
    //Copy Verteces into Above Buffer Using MAPPING
    D3D11_MAPPED_SUBRESOURCE mappedResource;
    /* ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferRectanglePos, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, rectVertices, sizeof(rectVertices));
    gpID3D11DeviceContext->Unmap(gpVertexBufferRectanglePos, 0);
 */
    //Color Rectangle!
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(rectTexCoords);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferRectangleTex);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Rectangle Texture!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Rectangle Texture Successful!!.\n");
        fclose(fptr);
    }
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferRectangleTex, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, rectTexCoords, sizeof(rectTexCoords));
    gpID3D11DeviceContext->Unmap(gpVertexBufferRectangleTex, 0);
    ...
    gpID3D11DeviceContext->RSSetState(gpRasterizerState);
        ...
        //Pyramid!.
    hr = loadD3DTexture(&gpSRVRectTex);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "loadD3DTexture for Rectangle Texture Failed!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "loadD3DTexture for Rectangle Texture Successful!!.\n");
        fclose(fptr);
    }
    ...
    //CreateSamplerState
    gpID3D11Device->CreateSamplerState(&samplerDesc, &gpSamplerStateRectTex);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateSamplerState for Rectangle Texture Failed!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateSamplerState for Rectangle Texture Successful!!.\n");
        fclose(fptr);
    }
...
void makeCheckImage(void) {
    ...
    for(i = 0; i < CHECKER_HEIGHT; i++) {
        for(j = 0; j < CHECKER_WIDTH; j++) {
            ...
            CheckerBoardData[(i * CHECKER_HEIGHT + j) * 4 + 0] = c;
            CheckerBoardData[(i * CHECKER_HEIGHT + j) * 4 + 1] = c;
            CheckerBoardData[(i * CHECKER_HEIGHT + j) * 4 + 2] = c;
            CheckerBoardData[(i * CHECKER_HEIGHT + j) * 4 + 3] = 255;
        }
    }
}

HRESULT loadD3DTexture(ID3D11ShaderResourceView **ppTexShaderRes) {
    HRESULT hr;
    void makeCheckImage(void);

    makeCheckImage();

    // Create Texture Holdings
    ID3D11Texture2D *texture = NULL;
    D3D11_TEXTURE2D_DESC texDesc;
    ZeroMemory(&texDesc, sizeof(texDesc));
    texDesc.Width = CHECKER_WIDTH;
    texDesc.Height = CHECKER_HEIGHT;
    texDesc.MipLevels = 1;
    texDesc.ArraySize = 1;
    ...
    subData.pSysMem = CheckerBoardData;
    subData.SysMemPitch = CHECKER_WIDTH * 4;

    hr = gpID3D11Device-> CreateTexture2D(&texDesc, &subData, &texture);
    if(FAILED(hr)){
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "CreateTexture2D Failed.\n");
        fclose(fptr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "CreateTexture2D Sceesssful!.\n");
        fclose(fptr);
    }
    ...
    hr = gpID3D11Device -> CreateShaderResourceView(texture, &SRVDesc, ppTexShaderRes);
    if(FAILED(hr)){
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "CreateShaderResourceView For Checker Board Failed.\n");
        fclose(fptr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "CreateShaderResourceView For Checker Board Sceesssful!.\n");
        fclose(fptr);
    }
    return(hr);
}
...
void setRectBufferData(int iPos) {
    ...
    if(iPos == 1) {
        ...
        rectVertices[8] = 0.0f;
        rectVertices[9] = -2.0f;
        rectVertices[10] = -1.0f;
        rectVertices[11] = 0.0f;
        rectVertices[12] = 0.0f;
        rectVertices[13] = 1.0f;
        rectVertices[14] = 0.0f;
        rectVertices[15] = 0.0f;
        rectVertices[16] = -1.0f;
        rectVertices[17] = 0.0f;
        ...
        rectVertices[1] = 1.0f;
        rectVertices[2] = 0.0f;
        rectVertices[3] = 2.41421f;
        rectVertices[4] = 1.0f;
        rectVertices[5] = 1.41421f;
        rectVertices[6] = 1.0f;
        rectVertices[7] = -1.0f;
        rectVertices[8] = 0.0f;
        rectVertices[9] = 1.0f;
        rectVertices[10] = -1.0f;
        rectVertices[11] = 0.0f;
        rectVertices[12] = 2.41421f;
        rectVertices[13] = 1.0f;
        rectVertices[14] = 1.41421f;
        rectVertices[15] = 2.41421f;
        rectVertices[16] = -1.0f;
        rectVertices[17] = 1.41421f;
...
void display(void){
    ...
    UINT iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferRectanglePos, &iStride, &iOffset);

    iStride = sizeof(float) * 2;
    iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferRectangleTex, &iStride, &iOffset);

    //Binding PixelShader Resources!.
    gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpSRVRectTex);
    gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpSamplerStateRectTex);
    ...
    translateMat = XMMatrixTranslation(0.0f, 0.0f, 3.5f);   //Postive Z Value Since it follows Left Hand Rule!
...
void uninitialize(void) {
    ...
    //Code!.
    if(gpSamplerStateRectTex) {
        gpSamplerStateRectTex->Release();
        gpSamplerStateRectTex = NULL;
    }

    if(gpSRVRectTex) {
        gpSRVRectTex->Release();
        gpSRVRectTex = NULL;
    }
    ...
    if(gpVertexBufferRectanglePos){
        gpVertexBufferRectanglePos->Release();
        gpVertexBufferRectanglePos = NULL;
    }

    if(gpVertexBufferRectangleTex){
        gpVertexBufferRectangleTex->Release();
        gpVertexBufferRectangleTex = NULL;
    }
