...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_DiffuseLightOnSphereLog.txt";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
    ...
    hwnd = CreateWindow(szClassName,
                    TEXT("D3D :: Diffuse Light On Sphere!"),
                    WS_OVERLAPPEDWINDOW,
                    100,
                    100,
                    WIN_WIDTH,
...
HRESULT initialize(void) {
    ...
    const char* vertexShaderSourceCode =
        ...
        "   if(lPressed == 1) {"    \
        "       float4 eyeCoords = mul(worldViewMat, pos);"   \
        "       float3 tNorm = normalize(mul((float3x3)worldViewMat, (float3)normal));"   \
        "       float3 s = (float3)normalize(lightPos - eyeCoords);"    \
        "       vertOut.diffuseLight = ld * kd * max(dot(s, tNorm), 0.0);"  \
        "   }"
        "   else {" 
        "       vertOut.diffuseLight = float4(1.0, 1.0, 1.0, 1.0);" \
        "   }"
        "   vertOut.position = mul(projMat, mul(worldViewMat, pos));"   \
        "   return(vertOut);"  \
        "}";
    ...
    const char* pixelShaderSourceCode = 
        "float4 main(float4 position: SV_POSITION, float4 diffuleLightColor: COLOR) : SV_TARGET {" \
        "   return diffuleLightColor;"  \
        "}";
