...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_GeometryShaderLog.txt";
...
ID3D11GeometryShader *gpGeometryShader = NULL;
...
HRESULT initialize(void) {
    ...
    const char* geometryShaderSourceCode = 
        ...
        "   geometryOutput geomatOut;"  \
        "   for(int vertex = 0; vertex < 3; vertex++) {"    \
        "       if(vertex == 0) {"  \
        "           geomatOut.color = float4(1.0, 1.0, 0.0, 1.0);" \
        "       }"  \
        "       else if(vertex == 1) {" \
        "           geomatOut.color = float4(0.0, 1.0, 1.0, 1.0);" \
        "       }"  \
        "       else if(vertex == 2) {" \
        "           geomatOut.color = float4(1.0, 0.0, 1.0, 1.0);" \
        "       }"  \
        "       geomatOut.position = mul(worldViewProjMat, (input[vertex].position + float4(0.0, 1.0, 0.0, 0.0)));"     \
        "       triangleStream.Append(geomatOut);"  \

        "       geomatOut.position = mul(worldViewProjMat, (input[vertex].position + float4(1.0, -1.0, 0.0, 0.0)));" \
        "       triangleStream.Append(geomatOut);"  \
    ...
    ID3DBlob *pGeometryShaderCode = NULL;
    pErrorBlob = NULL;
    ...
    if(FAILED(hr)){
        if(pErrorBlob != NULL){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "D3DCompile Failed While Compiling Geometry Shader!.\n");
            fprintf(fptr, "Error:: %s", (char*)pErrorBlob->GetBufferPointer());
            fclose(fptr);
            pErrorBlob->Release();
            pErrorBlob = NULL;
        ...
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "Geometry Shader Compilation Successful!.\n");
        fclose(fptr);
    }

    hr = gpID3D11Device->CreateGeometryShader(pGeometryShaderCode->GetBufferPointer(), pGeometryShaderCode->GetBufferSize(), NULL, &gpGeometryShader);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateGeometryShader Failed!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateGeometryShader Successful!!.\n");
        fclose(fptr);
    }
    gpID3D11DeviceContext->GSSetShader(gpGeometryShader, 0, 0);
    ...
    pGeometryShaderCode->Release();
    pVertexShaderCode = NULL;
    pPixelShaderCode = NULL;
    pGeometryShaderCode = NULL;
    ...
    gpID3D11DeviceContext->GSSetConstantBuffers(0, 1, &gpConstantBuffer);
...
void display(void){
    ...
    CBUFFER cBuffer;
    cBuffer.WorldViewProjMat = worldViewProjMat;
