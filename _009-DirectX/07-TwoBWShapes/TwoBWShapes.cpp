...
ID3D11Buffer *gpVertexBufferRectangle = NULL;
ID3D11Buffer *gpVertexBufferTriangle = NULL;
...
HRESULT initialize(void) {
    ...
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferRectangle);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Rectangle!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Rectangle Successful!!.\n");
        fclose(fptr);
    }
    ...
    gpID3D11DeviceContext->Map(gpVertexBufferRectangle, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, rectVertices, sizeof(rectVertices));
    gpID3D11DeviceContext->Unmap(gpVertexBufferRectangle, 0);
    ...
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferTriangle);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Triangle!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Triangle Successful!!.\n");
        fclose(fptr);
    }

    //Copy Verteces into Abover Buffer Using MAPPING
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferTriangle, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, triangleVert, sizeof(triangleVert));
    gpID3D11DeviceContext->Unmap(gpVertexBufferTriangle, 0);
...
void display(void){
    ...
    UINT iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferRectangle, &iStride, &iOffset);
    ...
    iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferTriangle, &iStride, &iOffset);
...
void uninitialize(void) {
    ...
    //Code!.
        ...
        if(gpVertexBufferRectangle){
        gpVertexBufferRectangle->Release();
        gpVertexBufferRectangle = NULL;
    }
