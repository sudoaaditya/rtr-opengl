...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_ColoredShapesLog.txt";
...
ID3D11Buffer *gpVertexBufferRectangleCol = NULL;

ID3D11Buffer *gpVertexBufferTrianglePos = NULL;
ID3D11Buffer *gpVertexBufferTriangleCol = NULL;

float fRotAngleRect = 0.0f;
float fRotAngleTriangle = 0.0f;
...
HRESULT initialize(void) {
    ...
    inputElementsDesc[1].InputSlot = 1; // AMC_ATTRIBUTE_COLOR = 1
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(rectCol);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferRectangleCol);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Rectangle Color!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Rectangle Color Successful!!.\n");
        fclose(fptr);
    }
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferRectangleCol, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, rectCol, sizeof(rectCol));
    gpID3D11DeviceContext->Unmap(gpVertexBufferRectangleCol, 0);
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(triangleVert);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferTrianglePos);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Triangle Position!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Triangle Position Successful!!.\n");
        fclose(fptr);
    }
    //Copy Verteces into Abover Buffer Using MAPPING
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferTrianglePos, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, triangleVert, sizeof(triangleVert));
    gpID3D11DeviceContext->Unmap(gpVertexBufferTrianglePos, 0);

    //Triangle COlor
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(triangleCol);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    //Create Buffer!
    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferTriangleCol);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
            fprintf(fptr, "ID3D11Device::CreateBuffer Failed For Vertex Buffer For Triangle Color!!.\n");
            fclose(fptr);
            return(hr);
    }
    else {
        fopen_s(&fptr, szLogFileName, "a+");
        fprintf(fptr, "ID3D11Device::CreateBuffer For Vertex Buffer For Triangle Color Successful!!.\n");
        fclose(fptr);
    }
    //Copy Verteces into Abover Buffer Using MAPPING
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferTriangleCol, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, triangleCol, sizeof(triangleCol));
    gpID3D11DeviceContext->Unmap(gpVertexBufferTriangleCol, 0);
...
void display(void){
    ...
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferRectangleCol, &iStride, &iOffset);
    ...
    translateMat = XMMatrixTranslation(-1.5f, 0.0f, 5.0f);   //Postive Z Value Since it follows Left Hand Rule!
    rotationMat = XMMatrixRotationY(-fRotAngleRect);
    ...
    iOffset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpVertexBufferTrianglePos, &iStride, &iOffset);
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferTriangleCol, &iStride, &iOffset);
    ...
    translateMat = XMMatrixTranslation(1.5f, 0.0f, 5.0f);
    rotationMat = XMMatrixRotationY(-fRotAngleTriangle);
    worldMat *= rotationMat * translateMat;
    worldViewProjMat = worldMat * viewMat * perspectiveProjMat;
    cBuffer.WorldViewProjectionMatrix = worldViewProjMat;
    gpID3D11DeviceContext->UpdateSubresource(gpConstantBuffer, 0, NULL, &cBuffer, 0, 0);
...
void update(void) {
    fRotAngleRect += 0.005f;
    if(fRotAngleRect >= 360.0f){
        fRotAngleRect = 0.0f;
    }

    fRotAngleTriangle += 0.005f;
    if(fRotAngleTriangle >= 360.0f){
        fRotAngleTriangle = 0.0f;
    }
}
...
void uninitialize(void) {
    ...
    if(gpVertexBufferRectangleCol){
        gpVertexBufferRectangleCol->Release();
        gpVertexBufferRectangleCol = NULL;
    }
    
    if(gpVertexBufferTrianglePos){
        gpVertexBufferTrianglePos->Release();
        gpVertexBufferTrianglePos = NULL;
    }

    if(gpVertexBufferTriangleCol){
        gpVertexBufferTriangleCol->Release();
        gpVertexBufferTriangleCol = NULL;
    }
