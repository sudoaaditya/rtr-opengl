...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_OrthoTriangleLog.txt";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
    ...
    hwnd = CreateWindow(szClassName,
                    TEXT("Orthographic Triangle D3D"),
                    WS_OVERLAPPEDWINDOW,
                    100,
                    100,
                    WIN_WIDTH,
...
HRESULT initialize(void) {
    ...
    orthographicProjMat = XMMatrixIdentity();
...
HRESULT resize(int width, int height) {
    ...
    //Projection matrix!.
    if(width <= height) {
        orthographicProjMat = XMMatrixOrthographicOffCenterLH(
            -100.0f, 
            100.0f,
...
void display(void){
    ...
    worldViewProjMat = worldMat * viewMat * orthographicProjMat;
