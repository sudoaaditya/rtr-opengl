...
bool bVertLight = true;
bool bPixelLight= false;
...
struct CBUFFER {
    ...
    unsigned int VertLight;
    unsigned int PixelLight;
};
        ...
        case WM_KEYDOWN: 
            switch(wParam) {
                ...
                case 'p':
                    ...
                    fopen_s(&fptr, szLogFileName, "a+");
                    fprintf(fptr, "Light P, %d, %d.\n", bPixelLight, bVertLight);
                    fclose(fptr);
                    break;
...
HRESULT initialize(void) {
    ...
    const char* vertexShaderSourceCode =
        ...
        "       if(vertLight == 1) {"   \
        "          float3 n_lightDir = normalize(vertOut.lightDir);"  \
        "          float3 n_tNorm = normalize(vertOut.tNorm);"    \
        "          float3 n_viewerVec = normalize(vertOut.viewerVec);"    \
        ...
        "          vertOut.phong_ads_color = ambient + diffuse + specular;"    \
        "       }"  \
        "       else {" \
        "           vertOut.phong_ads_color = float4(1.0, 1.0, 1.0, 1.0);   "   \
        "       }"  \
        "   }"  \
    ...
    const char* pixelShaderSourceCode = 
        ...
        "   }"  \
        "   else {" \
        "       phong_ads_color = pixelIn.phong_ads_color;"  \
        "   }"  \
...
void display(void){
    ...
    if(bLighting == true) {
        if(bPixelLight == true) {
            cBuffer.PixelLight = 0;
            cBuffer.VertLight = 0;
        }
        else if(bVertLight == true){
            cBuffer.VertLight = 0;
            cBuffer.PixelLight = 0;
        }
        ...
        cBuffer.Ld = XMVectorSet(0.5f, 0.2f, 0.7f, 1.0f);
        cBuffer.Ls = XMVectorSet(0.7f, 0.0f, 0.0f, 1.0f);
        ...
        cBuffer.LightPos = XMVectorSet(100.0f, 100.0f, -100.0f, 1.0f);
    }
    else {
