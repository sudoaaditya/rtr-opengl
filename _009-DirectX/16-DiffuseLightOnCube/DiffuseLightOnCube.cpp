...
//GLobal Vars!.
FILE *fptr = NULL;
const char *szLogFileName = "_DiffuseLightLog.txt";
...
ID3D11Buffer *gpVertexBufferCubeNorm = NULL;
...
HRESULT initialize(void) {
    ...
    ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
    vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeNormals);
    vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

    hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpVertexBufferCubeNorm);
    if(FAILED(hr)){
            fopen_s(&fptr, szLogFileName, "a+");
    ...
    ZeroMemory(&mappedResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpID3D11DeviceContext->Map(gpVertexBufferCubeNorm, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    memcpy(mappedResource.pData, cubeNormals, sizeof(cubeNormals));
    gpID3D11DeviceContext->Unmap(gpVertexBufferCubeNorm, 0);
...
void display(void){
    ...
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpVertexBufferCubeNorm, &iStride, &iOffset);
    ...
    translateMat = XMMatrixTranslation(0.0f, 0.0f, 5.0f);   //Postive Z Value Since it follows Left Hand Rule!
...
void uninitialize(void) {
    ...
    if(gpVertexBufferCubeNorm){
        gpVertexBufferCubeNorm->Release();
        gpVertexBufferCubeNorm = NULL;
    }
