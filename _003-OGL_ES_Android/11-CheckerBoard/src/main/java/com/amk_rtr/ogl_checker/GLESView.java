package com.amk_rtr.ogl_checker;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    //Checker vars
    private int checkerWidth = 64;
    private int checkerHeight = 64;
    private byte checkerBoardData[] = new byte[checkerWidth*checkerHeight*4];
    private int texChecker[] = new int[1];
    ...
    private void initialize() {
        ...
        if(texChecker[0] == 0) {
            ...
            System.out.println("RTR: Texture Checker Created And Loaded Successfully!!");
        }
    ...
    private void makeCheckerIamge() {
        ...
        for(i = 0; i < checkerHeight; i++) {
            for(j = 0; j < checkerWidth; j++) {
                ...
                checkerBoardData[(i * 64 + j) * 4 + 0] = (byte)c;
                checkerBoardData[(i * 64 + j) * 4 + 1] = (byte)c;
                checkerBoardData[(i * 64 + j) * 4 + 2] = (byte)c;
                checkerBoardData[(i * 64 + j) * 4 + 3] = (byte)0xff;//255; 
            }
        }
    }
    ...
    private int loadTexture() {
        ...
        ByteBuffer byteCheckBuffer = ByteBuffer.allocateDirect(checkerWidth * checkerHeight * 4);
        System.out.println("RTR: Len"+checkerBoardData.length);
        byteCheckBuffer.order(ByteOrder.nativeOrder());
        byteCheckBuffer.put(checkerBoardData);
        byteCheckBuffer.position(0);
        
        Bitmap bitmap = Bitmap.createBitmap(checkerWidth, checkerHeight, Bitmap.Config.ARGB_8888);
        bitmap.copyPixelsFromBuffer(byteCheckBuffer);
    ...
    private void display() {
        ...
        float rectVertArr[] = new float[12];
        ...
        Matrix.translateM(translateMat, 0, 0.0f, 0.0f, -3.6f);
        ...
        GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, texChecker[0]);
        ...
        FloatBuffer posBuffOne = createFloatBuffer(rectVertArr);

        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_rectanglePos[0]);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (rectVertArr.length * 4), posBuffOne, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
        ...
        FloatBuffer posBuffTwo = createFloatBuffer(rectVertArr);

        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_rectanglePos[0]);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (rectVertArr.length * 4), posBuffTwo, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
        GLES32.glDrawArrays(GLES32.GL_TRIANGLE_FAN, 0, 4);
    ...
    private void uninitialize() {
        ...
        if(texChecker[0] != 0) {
            GLES32.glDeleteTextures(1, texChecker, 0);
            texChecker[0] = 0;
        }
