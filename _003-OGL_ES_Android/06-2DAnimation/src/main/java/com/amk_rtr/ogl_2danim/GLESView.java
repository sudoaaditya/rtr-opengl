package com.amk_rtr.ogl_2danim;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    private float fTriangleRotAngle = 0.5f;
    private float fRectangleRotAngle = 0.5f;
    ...
    private void display() {
        ...
        Matrix.translateM(translateMat, 0, -1.5f, 0.0f, -5.0f);
        Matrix.multiplyMM(modelViewMat, 0 , modelViewMat, 0, translateMat, 0); 

        Matrix.setRotateM(rotateMat, 0, fTriangleRotAngle, 0.0f, 1.0f, 0.0f);
        ...
        Matrix.setRotateM(rotateMat, 0, fRectangleRotAngle, 1.0f, 0.0f, 0.0f);
        Matrix.multiplyMM(modelViewMat, 0, modelViewMat, 0, rotateMat, 0);
    ...
    private void update() {
        ...
        fTriangleRotAngle += fOffset;
        if(fTriangleRotAngle >= 360.0f) {
            fTriangleRotAngle = 0.0f;
        }

        fRectangleRotAngle += fOffset;
        if(fRectangleRotAngle >= 360.0f) {
            fRectangleRotAngle = 0.0f;
        }
    }
