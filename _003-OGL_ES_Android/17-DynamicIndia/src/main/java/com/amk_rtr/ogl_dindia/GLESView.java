package com.amk_rtr.ogl_dindia;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener {
    ...
    private float modelViewProjMat[] = new float[16];
    private float translateMat[] = new float[16];

    //animation Vars
    private boolean bI1Reached = false, bNReached = false, bDReached = false,bI2Reached = false, bAReached = false, bPlaneReached = false;
    private float fTransSpeed = 0.003f;
    private float dArcCol[] = new float[1260];
    static float bXTransI = -0.8f;
    static float bXTransA = 0.8f;
    static float bYTransN = 2.0f;
    static float bYTransI = -2.0f;
    static float A = 0.0f;
	static float Sr=0.0f, Sg = 0.0f, Sb = 0.0f;
    ...
    private void initialize() {
        ...
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (1260 * 4), null, GLES32.GL_DYNAMIC_DRAW);
        ...
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (3*6*4), null, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glVertexAttribPointer(GLESMacros.AMC_ATTRIBUTE_COLOR, 3, GLES32.GL_FLOAT, false, 0, 0);
        GLES32.glEnableVertexAttribArray(GLESMacros.AMC_ATTRIBUTE_COLOR);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
        GLES32.glBindVertexArray(0);
    ...
    private void drawI1() {
        ...
        Matrix.translateM(translateMat, 0, bXTransI, 0.0f, -3.0f);
    ...
    private void drawN() {
        ...
        Matrix.translateM(translateMat, 0, 0.0f, bYTransN, -3.0f);
    ...
    private void drawD() {
        ...
        float colorArray[] = new float[]{Sr, Sg, Sb, Sr, Sg, Sb, Sr, Sg, Sb, Gr, Gg, Gb, Gr, Gg, Gb, Gr, Gg, Gb};
        
        //D
        GLES32.glBindVertexArray(vaoD[0]);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vboDCol[0]);
        FloatBuffer colBuffLine = createFloatBuffer(colorArray);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (colorArray.length * 4), colBuffLine, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);    
        ...
        GLES32.glBindVertexArray(vaoDArc[0]);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vboDArcCol[0]);
        FloatBuffer colBuffArc = createFloatBuffer(dArcCol);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (dArcCol.length * 4), colBuffArc, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
    ...
    private void drawI2() {
        ...
        Matrix.translateM(translateMat, 0, 0.0f, bYTransI, -3.0f);
    ...
    private void drawA() {
        ...
        Matrix.translateM(translateMat, 0, bXTransA, 0.0f, -3.0f);
    ...
    private void uninitialize() {
        ...
        if(vboAPos[0] != 0) {
            GLES32.glDeleteBuffers(1, vboAPos, 0);
            vboAPos[0] = 0;
        }
        if(vboACol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboACol, 0);
            vboACol[0] = 0;
        }
        if(vaoA[0] != 0) {
        ...
        if(vboNCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboNCol, 0);
            vboNCol[0] = 0;
        }
        if(vboNCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboNCol, 0);
        ...
        if(vboDCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboDCol, 0);
            vboDCol[0] = 0;
        }
        if(vboDCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboDCol, 0);
        ...
        if(vboDArcCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboDArcCol, 0);
            vboDArcCol[0] = 0;
        }
        if(vboDArcCol[0] != 0) {
            GLES32.glDeleteBuffers(1, vboDArcCol, 0);
        ...
        if(vaoDArc[0] != 0) {
            GLES32.glDeleteVertexArrays(1, vaoDArc, 0);
            vaoDArc[0] = 0;
        }
    
        if(vboI1Col[0] != 0) {
            GLES32.glDeleteBuffers(1, vboI1Col, 0);
            vboI1Col[0] = 0;
        }
        if(vboI1Col[0] != 0) {
            GLES32.glDeleteBuffers(1, vboI1Col, 0);
        ...
        if(vaoI1[0] != 0) {
            GLES32.glDeleteVertexArrays(1, vaoI1, 0);
            vaoI1[0] = 0;
        }
    
        if(vboI2Col[0] != 0) {
            GLES32.glDeleteBuffers(1, vboI2Col, 0);
            vboI2Col[0] = 0;
        }
        if(vboI2Col[0] != 0) {
            GLES32.glDeleteBuffers(1, vboI2Col, 0);
        ...
        if(vaoI2[0] != 0) {
            GLES32.glDeleteVertexArrays(1, vaoI2, 0);
            vaoI2[0] = 0;
        }
