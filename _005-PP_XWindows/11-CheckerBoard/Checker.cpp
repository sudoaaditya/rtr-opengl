...
GLuint vao_rectangle, vbo_rectangle, vbo_rectTex;

//Texture Vars
GLuint iCheckerTex = 0;
GLubyte checkerBoardData[Checker_Width][Checker_Height][4];
...
void CreateWindow(void) {
	...
	XStoreName(gpDisplay, gWindow, "Triangle Rectangle");
...
int initialize(void) {
    ...
    glBufferData(GL_ARRAY_BUFFER, (4*3*4), NULL, GL_DYNAMIC_DRAW);
	...
	if(loadTexture() == false) {
		...
		printf(" SUCCESS : Checker Texture Created & Loaded Successfully!..\n");
	}
...
bool loadTexture() {
	...
	if(makeCheckImage()) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 
				Checker_Width, Checker_Height, 0, GL_RGBA, 
				GL_UNSIGNED_BYTE, checkerBoardData);
	}
    return(bStatus);
}
...
bool makeCheckImage() {
	...
	for(i = 0;i < Checker_Height; i++) {
		for(j = 0; j < Checker_Width; j++) {
			...
			checkerBoardData[i][j][0] = (GLubyte)c;
			checkerBoardData[i][j][1] = (GLubyte)c;
			checkerBoardData[i][j][2] = (GLubyte)c;
			checkerBoardData[i][j][3] = (GLubyte)255;
