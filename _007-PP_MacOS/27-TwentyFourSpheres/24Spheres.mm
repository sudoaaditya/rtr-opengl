...
-(void) reshape {
    ...
    GLfloat width = iWinWidth = rect.size.width;
    GLfloat height = iWinHeight = rect.size.height;
...
-(void) drawView {
    ...
    if(bLighting == true) {
        ...
        [self pushLightPosition:iKeyPressed];
    ...
    //Call to  Update to increase angle!. but only when light is On!
    if(bLighting){
        [self update];
    }
...
-(void) draw24Spheres {
    ...
    for(i=0; i<4; i++) {
        ...
        for(j=0; j<6; j++) {
            if((i*6+j)<24) {
                [self pushMaterialData:materials[i*6+j] ];
            }
            [self drawSingleSphere];
