...
-(void) applicationDidFinishLaunching:(NSNotification *)aNotify {
    ...
    NSString *logFileNameWithPath = [NSString stringWithFormat:@"%@/_OrthoTriangleLog.txt", parentDirPath];
    ...
    [window setTitle:@"macOS:: Orthographic Triangle!."];
    [window center];
...
-(void) reshape {
    ...
    if(width < height) {
        ...
        orthographicProjMat = ortho((-100.0f * (width/height)),
                                    (100.0f * (width/height)),
                                    -100.0f,
                                    100.0f,
                                    -100.0f,
