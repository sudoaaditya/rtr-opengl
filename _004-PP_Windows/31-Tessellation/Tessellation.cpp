...
GLuint gTessControlShaderObj;
GLuint gTessEvaluationShaderObj;
GLuint vao, vbo;
GLuint mvpUniform;
GLuint numSegmentUniform;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow) {
    ...
    if(fopen_s(&fptr, "_TessShaderLog.txt", "w") != 0) {
...
int initialize(void) {
    ...
    iShaderInfoLogLen = 0;
    szInfoLog = NULL;
    gTessControlShaderObj = glCreateShader(GL_TESS_CONTROL_SHADER);

    const GLchar *tessControlShaderSource = 
        "#version 450 core" \
		"\n" \
    ...
    glShaderSource(gTessControlShaderObj, 1, (const GLchar**)&tessControlShaderSource, NULL);

    glCompileShader(gTessControlShaderObj);

    glGetShaderiv(gTessControlShaderObj, GL_COMPILE_STATUS, &iShaderCompileStatus);

    if(iShaderCompileStatus == GL_FALSE) {
        glGetShaderiv(gTessControlShaderObj, GL_INFO_LOG_LENGTH, &iShaderInfoLogLen);
        ...
        if(iShaderInfoLogLen > 0) {
            ...
            if(szInfoLog != NULL) {
                ...
                glGetShaderInfoLog(gTessControlShaderObj,
                                iShaderInfoLogLen,
                                &written,
                                szInfoLog);

                fprintf(fptr,"Tessellation Control Shader Log :\n %s\n",szInfoLog);
                free(szInfoLog);
                uninitialize();
                DestroyWindow(ghwnd);
                exit(0);
        ...
        fprintf(fptr, "Tessellation Control Shader Compiled Successfully!!..\n");
    }
    ...
    iShaderInfoLogLen = 0;
    szInfoLog = NULL;
    gTessEvaluationShaderObj = glCreateShader(GL_TESS_EVALUATION_SHADER);

    const GLchar *tessEvalShaderSource = 
        "#version 450 core" \
		"\n" \
		"layout(isolines)in;" \
    ...
    glShaderSource(gTessEvaluationShaderObj, 1, (const GLchar**)&tessEvalShaderSource, NULL);

    glCompileShader(gTessEvaluationShaderObj);

    glGetShaderiv(gTessEvaluationShaderObj, GL_COMPILE_STATUS, &iShaderCompileStatus);

    if(iShaderCompileStatus == GL_FALSE) {
        glGetShaderiv(gTessEvaluationShaderObj, GL_INFO_LOG_LENGTH, &iShaderInfoLogLen);
        ...
        if(iShaderInfoLogLen > 0) {
            ...
            if(szInfoLog != NULL) {
                ...
                glGetShaderInfoLog(gTessEvaluationShaderObj,
                                iShaderInfoLogLen,
                                &written,
                                szInfoLog);

                fprintf(fptr,"Tessellation Evaluation Shader Log :\n %s\n",szInfoLog);
                free(szInfoLog);
                uninitialize();
                DestroyWindow(ghwnd);
                exit(0);
        ...
        fprintf(fptr, "Tessellation Evaluation Shader Compiled Successfully!!..\n");
    }
    ...
    if(iShaderCompileStatus == GL_FALSE) {
        ...
        fprintf(fptr, "Fragment ShaderCompiled Successfully!!..\n");
    }
    ...
    glAttachShader(gProgramShaderObj, gTessControlShaderObj);
    glAttachShader(gProgramShaderObj, gTessEvaluationShaderObj);
    ...
    numSegmentUniform = glGetUniformLocation(gProgramShaderObj, "numberOfSegments");
    numStripsUniform = glGetUniformLocation(gProgramShaderObj, "numberOfStrips");
    lineColorUniform = glGetUniformLocation(gProgramShaderObj, "lineColor");


    GLfloat lineVert[] = { -1.0f, -1.0f, -0.5f, 1.0f, 0.5f, -1.0f, 1.0f, 1.0f };
...
void display(void) {
    ...
    glUniform1i(numSegmentUniform, numLineSegments);

    TCHAR str[255];
    wsprintf(str, TEXT("OGLPP Window : [ Segments :: %d ]"), numLineSegments);
    SetWindowText(ghwnd, str);

    glUniform1i(numStripsUniform, 1);
    glUniform4fv(lineColorUniform, 1, vec4(0.5f, 0.2f, 0.7f, 1.0f));
...
void uninitialize(void) {
    ...
    glDetachShader(gProgramShaderObj, gTessControlShaderObj);
    glDetachShader(gProgramShaderObj, gTessEvaluationShaderObj);
    glDetachShader(gProgramShaderObj, gVertexShaderObj);

    glDeleteShader(gFragmentShaderObj);
    gFragmentShaderObj = 0;

    glDeleteShader(gTessControlShaderObj);
    gTessControlShaderObj = 0;
    
    glDeleteShader(gTessEvaluationShaderObj);
    gTessEvaluationShaderObj = 0;
