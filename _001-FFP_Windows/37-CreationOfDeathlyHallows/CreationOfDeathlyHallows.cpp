...
void display(void){
	...
	static bool bToWrite = false, bTriReached = false, bCirReached = false;

	//for Triangle
	static GLfloat fTXtran=-2.5f,fTYtran=-2.5f,fTAngle=0.0f;

	//for circle
	static GLfloat fCXtran=2.5f,fCYtran=-2.5f,fCAngle=0.0f;

	//For Line
	static GLfloat fLYtran = 2.0f;
	...
	glLoadIdentity();
	glTranslatef(fTXtran,fTYtran,-3.0f);
	...
	if(bTriReached == true) {
		...
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(fCXtran,fCYtran,-3.0f);
		glRotatef(fCAngle,0.0f,1.0f,0.0f);
		glPointSize(2.0f);
		glBegin(GL_POINTS);
	...
	if(bCirReached) {
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glTranslatef(0.0f,fLYtran,-3.0f);
		glLineWidth(2.0f);
