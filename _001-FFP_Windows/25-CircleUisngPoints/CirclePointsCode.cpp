...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	WNDCLASSEX wndclass;
	HWND hwnd;
	TCHAR szApp[] = TEXT("Circle");
	int iRet = 0;
	bool bDone = false;

	if (fopen_s(&fptr, "PCirLog.txt", "w") != 0) {
...
void display(void) {
	...
	for (float angle = 0.0f; angle < 2 * PI; angle += 0.001f) {

		glVertex3f(0.8*cos(angle), 0.8*sin(angle), 0.0f);
	}
