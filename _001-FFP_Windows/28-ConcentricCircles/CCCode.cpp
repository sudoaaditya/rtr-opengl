...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevinstance, LPSTR lpszCmdLine, int iCmdShow) {
	...
	if (fopen_s(&fptr, "ConCirLog.txt", "w") != 0) {
...
void display(void) {
	...
	GLfloat fxCord = 0.0f, fyCord = 0.0f, fAngle = 0.0f, fRad = 1.0f;
	...
	for (int j = 0; j < 8; j++) {
		...
		glColor3f(fR, fG, fB);
		for (int i = 0; i < LineToDrawCircle; i++) {


			fAngle = (GLfloat)((2.0f * M_PI * i) / LineToDrawCircle);
			fxCord = (GLfloat)cos(fAngle);
			fyCord = (GLfloat)sin(fAngle);
			glVertex3f(fRad*fxCord, fRad*fyCord, 0.0f);
