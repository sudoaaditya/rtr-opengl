...
void display(void) {
	...
	static float fx = 0.0f, fy = 0.8f, fxPrev = 0.0f, fyPrev = 0.8f, r = 0.3f;
	...
	//ground
	glBegin(GL_POLYGON);
	glColor3f(0.86f, 0.54f, 0.12f);
	glVertex3f(1.0f, -0.035f, 0.0f);
	glVertex3f(-1.0f, -0.035f, 0.0f);
	...
	//glPointSize(7.0f);
	glBegin(GL_LINES);
	glColor3f(0.9607f, 0.317f, 0.039f);
	for (int i = 0; i < 360; i++) {
		glVertex3f(fxPrev, fyPrev, 0.0f);
		glVertex3f((float)(fx + r * cos(i)), (float)(fy + r * sin(i)), 0.0f);
		//fxPrev = (float)(fx + r * cos(i));
		//fyPrev = (float)(fy + r * sin(i));
