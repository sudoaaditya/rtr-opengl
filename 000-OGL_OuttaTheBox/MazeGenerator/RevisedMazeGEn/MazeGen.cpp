...
Box** Box::CheckNeighbour(){
    ...
    if(retI != -1) {
        ...
        if(!(*right) && right->visitedBox == false ){
    ...
    if(retI != -1) {
        ...
        if(!(*bottom) && bottom->visitedBox == false  ){
    ...
    if(temp.size() <= 0) {
        ...
        int rIndx = floor(Random(0, temp.size()-1));
        fprintf(fptr,"TEMP SIZE : %d\t %d\n",temp.size(), rIndx);
        fflush(fptr);
        fprintf(fptr,"X:%d\tY:%d\tVisit:%d\n",(*temp[rIndx])->x,(*temp[rIndx])->y,(*temp[rIndx])->visitedBox);
        fflush(fptr);
        return(temp.at(rIndx));
    }
}
